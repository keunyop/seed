...
## 2026-09-01 초등부 사진으로 AWANA 아이 사진 동기화

### 안전 기준

- 운영 DB에 저장된 이름 문자열이 초등부와 AWANA에 각각 정확히 1행씩 존재할 때만 같은 아이로 간주했다.
- 어느 그룹에서든 같은 이름이 2행 이상이면 동명이인으로 보고 해당 이름 전체를 제외했다.
- 초등부 사진이 null·빈 값이거나 이미지 Data URL이 아니면 AWANA의 기존 사진을 지우지 않고 제외했다.
- PATCH는 기본 조직, `ministry_group = awana`, 대상 ID, 이름, 실행 직전 `updated_at`을 모두 조건으로 걸고 `photo_data_url` 한 필드만 변경했다.

### 운영 데이터 변경

- 변경 전 전체 조회: 초등부 71행, AWANA 53행, 두 그룹 공통 이름 32개를 확인했다.
- `김엘리`는 초등부 2행·AWANA 2행의 동명이인이어서 4행 모두 변경하지 않았다.
- 초등부 사진이 없는 공통 이름 8명(`김도완`, `김선우`, `변시원`, `송리유`, `송지아`, `정시아`, `조샤론`, `조의겸`)은 변경하지 않았다.
- 안전 조건을 충족한 23명의 AWANA `children.photo_data_url`만 초등부 사진으로 갱신했다.
- 23명 중 22명은 기존 AWANA 사진이 비어 있었고, `김태오` 1명은 기존 AWANA 사진을 초등부 사진으로 교체했다.
- 이름, 반, 보호자, 출석, 활성 상태와 다른 테이블은 변경하지 않았다.

### 검증

- 실행 전 dry-run: 갱신 후보 23명, 동명이인 제외 1개 이름, 초등부 사진 없음 제외 8명 확인.
- 제한된 PATCH: 23건 모두 예상한 정확히 1행을 반환했고 대상 ID·이름·그룹·사진 일치를 확인했다.
- 실행 직후 전체 재조회: 23명의 초등부/AWANA 사진 값이 모두 일치하고 대상 외 기존 행의 사진 SHA-256이 변경되지 않았음을 확인했다.
- 독립 후속 dry-run: `alreadyMatchingNames` 23명, `updateCandidates` 0명, 동명이인·사진 없음 제외 목록이 변경 전과 같음을 확인했다.
- `node --check tmp\sync-awana-photos-by-unique-name.mjs`: 통과.

### 데이터/배포 영향

- 운영 Supabase 데이터에 즉시 반영됐다. 앱을 이미 열어 둔 경우 새로고침하면 AWANA 아바타에 반영된다.
- 앱 코드, migration, 환경변수, 패키지와 배포 변경은 없다.
- 실행 중 검증 실패 시 메모리에 보관한 직전 사진으로 자동 롤백하도록 했으며 실제 실행은 롤백 없이 성공했다.

## 2026-08-31 AWANA 2026-09-04 시작일 적용

### 제품 결정

- AWANA 시작일은 2026년 9월 4일로 고정하고, 그 이전 출석 세션은 운영 DB에서 삭제하지 않은 채 화면과 보상 집계에서 제외한다.
- 초등부 날짜·월간 현황은 기존 동작을 유지한다.

### 완료

- 공용 AWANA 시작일 상수와 그룹별 기본 출석 날짜 helper를 추가했다. 현재 날짜의 가장 가까운 금요일이 시작일보다 이르면 2026-09-04를 사용한다.
- AWANA 일별 날짜 입력에 2026-09-04 하한을 적용하고 시작일 전 입력을 시작일로 보정했다.
- AWANA `전체 현황`의 날짜 목록과 아이별 출석·암송 합계에서 2026-08-28을 포함한 시작일 이전 세션을 제외했다.
- AWANA 다이아몬드 진행도 시작일 이후의 서로 다른 `present` 날짜만 계산하도록 조정했다.
- 시작일 전 fixture와 시작일 이후 출석을 함께 둔 단위·mock E2E로 날짜 목록, 합계, 다이아몬드 경계를 검증했다.

### 검증

- `pnpm run typecheck`: 통과
- `pnpm run lint`: 통과
- `pnpm run test`: 10 files, 68 tests 통과
- `pnpm run test:db`: 1 file, 1 test 통과
- `pnpm run build`: 통과
- `pnpm run test:e2e:mock`: Chromium mobile/desktop, WebKit mobile 총 9 tests 통과
- mock E2E에서 AWANA 일별 기본 날짜와 최소 날짜가 2026-09-04이고, 전체 현황에 2026-08-28이 없으며 9월 4일이 표시되는 것을 확인했다.
- 같은 E2E에서 시작일 이후 출석 3회→9월 4일 출석 체크 후 4회·다이아몬드 1개 완성과 390px 모바일 가로 overflow 없음도 확인했다.

### 데이터/배포 영향

- Supabase 스키마, 환경 변수, 패키지와 운영 데이터 변경은 없다. 기존 2026-08-28 row가 있다면 그대로 보존된다.
- UI와 집계 변경 반영을 위해 앱 배포가 필요하다.

## 2026-08-31 최초 로그인 부서 선택과 AWANA 다이아몬드 3개

### 제품 결정

- 최초 선생님 로그인 모달에서 `초등부`/`AWANA` 부서를 먼저 선택하고 해당 그룹의 활성 선생님을 선택한다. 모달의 부서 선택은 상단 전역 그룹 선택과 같은 상태를 사용한다.
- AWANA 출석 다이아몬드는 모바일 가독성을 위해 20px 크기 3개로 표시한다. 출석 1회당 다음 다이아몬드 1/4, 4회당 1개 완성 규칙은 유지하고 12회에서 3개 완성 상태로 고정한다.

### 완료

- 로그인 모달에 최소 44px 높이의 부서 선택기를 추가하고 현재 선택 상태를 `aria-pressed`와 색상으로 구분했다.
- 모달에서 부서를 바꾸면 선택 그룹 store와 활성 선생님 목록을 다시 불러오며, 로딩·선생님 없음 상태에서도 로그인 흐름이 유지되도록 했다.
- 그룹별 마지막 로그인 교사 캐시와 비밀번호 없는 기존 로그인 흐름을 유지했다.
- AWANA 일별 체크와 전체 현황의 공용 다이아몬드 컴포넌트를 3개·20px·12회 상한으로 조정했다.
- 다이아몬드 컴포넌트·통계 단위 테스트와 최초 로그인 그룹 전환 mock E2E를 갱신했다.

### 검증

- `pnpm run typecheck`: 통과
- `pnpm run lint`: 통과
- `pnpm run test`: 10 files, 67 tests 통과
- `pnpm run test:db`: 1 file, 1 test 통과
- `pnpm run build`: 통과
- `pnpm run test:e2e:mock`: Chromium mobile/desktop, WebKit mobile 총 9 tests 통과
- mock E2E에서 최초 로그인 모달의 초등부→AWANA 선생님 목록 교체, 모달 유지, 초등부 복귀 로그인과 390px 모바일 가로 overflow 없음을 확인했다.

### 데이터/배포 영향

- Supabase 스키마, 환경 변수, 패키지와 운영 데이터 변경은 없다.
- UI 변경 반영을 위해 앱 배포가 필요하다.

## 2026-08-31 AWANA 원아 등록, 금요일 출석, 다이아몬드 진행

### 제품 결정

- AWANA 일별 출석 기본 날짜는 가장 가까운 금요일로 설정하고, 초등부는 기존 가장 가까운 일요일을 유지한다.
- AWANA에서는 출석 아이 한 줄 메모, 이번 주 메모, 통계 통합 메모를 표시하지 않는다. 기존 메모 데이터와 초등부 메모 기능은 보존한다.
- 서로 다른 날짜의 출석 1회마다 아이 이름 옆 다음 다이아몬드를 1/4씩 Sky Blue로 채운다. 4회에 1개가 완성되고 16회 이상은 4개 완성 상태로 유지하며, 현재 편집 중인 날짜의 상태도 즉시 반영한다.

### 운영 데이터 등록

- 쓰기 전 조회에서 요청 명단 48명, 대상 반 9개 유일성, 명단 내부 중복 없음, 기존 반·이름 충돌 0명을 확인했다.
- 운영 Supabase 기본 조직의 AWANA `children`에 `child-awana-20260831-001`~`child-awana-20260831-048` 48명만 추가했다.
- 반별 등록 결과: 스팍스-이근엽 5명, 스팍스-김민소 5명, 스팍스-백선희 4명, 스팍스-나준호 4명, 티앤티 5개 반 각 6명이다.
- 기존 `스팍스 - 유미미` 5명을 유지해 AWANA 활성 아이 총원은 53명이다.
- 제공되지 않은 생년월일·보호자·연락처·특이사항은 생성하지 않았고, `child_parents`와 기존 출석 데이터는 변경하지 않았다.

### 완료

- 사역 그룹별 출석 날짜 상태를 분리하고 AWANA는 금요일, 초등부는 일요일로 초기화했다.
- AWANA 주간 통계의 최근 4주와 전체 52주도 금요일 버킷을 사용하도록 일반화했다.
- AWANA 일별 카드의 아이 메모·이번 주 메모와 통계 통합 메모를 조건부로 숨겼다.
- 일별 체크와 AWANA 전체 현황에 4개 출석 다이아몬드를 추가하고, 각 다이아몬드가 4회 출석으로 1/4씩 채워지도록 했다. 접근성 이름에는 출석 횟수·완성 개수·현재 분기 진행을 제공한다.
- 날짜별 출석 진행 계산에 현재 화면의 출석 토글을 합쳐 저장 응답을 기다리는 동안에도 즉시 채움 상태가 보이게 했다.

### 검증

- 등록 후 별도 원격 재조회: 명단 48명의 반·성별·활성 상태가 모두 정확하고 ID 001~048이 모두 존재함을 확인했다.
- 등록 후 원격 총원/반별 재조회: AWANA 활성 53명, 기존 반 5명과 대상 반별 5·5·4·4·6·6·6·6·6명 확인.
- `pnpm run lint`: 통과
- `pnpm run typecheck`: 통과
- `pnpm run test`: 10 files, 67 tests 통과
- `pnpm run test:db`: 1 file, 1 test 통과
- `pnpm run build`: 통과
- `pnpm run test:e2e:mock`: Chromium mobile/desktop, WebKit mobile 총 9 tests 통과
- mock E2E에서 AWANA 기본 금요일, 출석 3회 첫 다이아몬드 3/4→출석 4회 다이아몬드 1개 완성, 출석·통계 메모 미표시, 390px 모바일 가로 overflow 없음과 초등부 메모 회귀를 확인했다.

### 데이터/배포 영향

- 원아 48명 등록은 운영 데이터에 즉시 반영됐다. 앱 UI 변경은 배포 후 적용되며 새 migration·환경변수·패키지는 없다.
- 등록 롤백이 필요하면 이번 작업의 결정적 ID 범위 `child-awana-20260831-001`~`048`만 대상으로 삼을 수 있다.

## 2026-08-31 AWANA 홈 반 등록 순서 정리

### 제품 결정

- AWANA 홈 화면의 반은 이름 접두어 기준으로 `커비스 → 스팍스 → 티앤티` 묶음 순서로 표시한다.
- 같은 묶음 안에서는 기존 등록 상대 순서를 유지한다.

### 완료

- 운영 Supabase 기본 조직의 AWANA `classes.sort_order`만 조정했다.
- `커비스 - 김영혜`, `커비스 - 이은정`을 0~1번으로, `스팍스 - 유미미`를 2번으로 옮겼다.
- 나머지 스팍스와 티앤티 반은 기존 상대 순서와 3~11번 값을 유지했다.
- 반 이름, 담임, 아이 배정, 출석 데이터와 초등부 반 순서는 변경하지 않았다.

### 검증

- 변경 전 원격 조회: AWANA 첫 순서가 `스팍스 - 유미미`, 이후 커비스 2개로 섞여 있음을 확인했다.
- `organization_id + ministry_group + id + 기존 sort_order`로 제한한 PATCH: 대상 3행만 각각 1행 갱신됨을 확인했다.
- 변경 후 원격 전체 조회: AWANA 12개 반이 `커비스 2개 → 스팍스 5개 → 티앤티 5개`, `sort_order` 0~11로 중복 없이 연속됨을 확인했다.
- 초등부 1~6학년의 ID와 `sort_order` 0~5가 변경 전과 같음을 확인했다.
- `pnpm run lint`: 통과
- `pnpm run typecheck`: 통과
- `pnpm test`: 9 files, 62 tests 통과
- `pnpm run test:db`: 1 file, 1 test 통과
- `pnpm run build`: 통과
- `pnpm run test:e2e:mock`: 직접 기동한 로컬 서버를 재사용해 Chromium mobile/desktop, WebKit mobile 총 9 tests 통과

### 데이터/배포 영향

- 운영 데이터에 즉시 반영했으며 코드, migration, 환경변수 변경은 없다.
- 이미 열려 있는 앱은 새로고침하면 변경된 홈 반 순서를 불러온다.
...
### 제품 기준
- 월간 현황은 선택 월의 모든 주일을 보여주고, 현재 선택 반에 소속된 활성 아이 기준으로 출석·큐티 횟수를 계산한다.
- 현재 DB에는 반 이동 당시의 반 snapshot이 없으므로 과거 소속 반 기준 집계는 이번 범위에서 제공하지 않는다.
- 아이별 날짜 메모는 100자 이하 최신값 하나만 유지하며, 댓글형 `이번 주 메모`와 별도로 저장한다.

### 완료
- `/attendance`에 `일별 체크`/`월간 현황` 전환과 월 선택을 추가했다.
- 전체 반에서 월간 현황으로 진입할 때 로그인 선생님의 담임 반이 있으면 해당 반을 우선 선택한다.
- 선택 월의 모든 주일을 2열 요약으로 표시하고 기록이 전혀 없는 날짜는 `기록 없음`으로 구분했다. 날짜를 누르면 해당 일별 체크로 이동한다.
- 아이별 월간 카드에 출석 횟수와 큐티 횟수를 표시하고, 펼친 날짜별 출석·큐티 버튼으로 과거 기록을 즉시 수정할 수 있게 했다.
- 일별 아이 카드에 한 줄 메모와 독립 저장/실패/재시도 상태를 추가했다. 입력 실패 시 내용을 유지한다.
- `attendance_records.note` 컬럼과 100자 DB 제약을 추가하는 `20260719000100_attendance_record_notes.sql` migration 및 generated type을 추가했다.
- 아이 메모는 note-only upsert, 출석·큐티는 해당 두 열만 upsert하도록 분리해 동시 요청이 서로의 최신값을 덮어쓰지 않게 했다.
- 통계 상세의 아바타 모드에서 사각 카드 래퍼를 제거하고 원형 아바타만 표시했다.
- 큐티 완료자 상세의 `총 N회 완료` 요약 문구를 제거하고 목록/복사의 아이별 완료 횟수는 유지했다.

### 검증
- 전체 ESLint: 통과
- `tsc --noEmit`: 통과
- 전체 단위/컴포넌트 테스트: 9 files, 60 tests 통과
- 출석 월간 계산·Supabase 저장 helper 집중 테스트: 2 files, 30 tests 통과
- DB migration 테스트: 1 file, 1 test 통과
- 통계 컴포넌트 테스트: 1 file, 4 tests 통과
- production build: 통과
- REST mock E2E: Chromium mobile/desktop, WebKit mobile에서 출석·통계 6 tests 통과
- E2E에서 아이 메모 note-only 요청, 첫 실패 후 입력 유지·재시도, 월간 과거 날짜 수정, 일별 이동, 390px 가로 overflow 없음, 통계 아바타 사각 카드 부재를 확인했다.

### 데이터/배포 영향과 사용자 작업
- 앱 배포 전에 `supabase/migrations/20260719000100_attendance_record_notes.sql`을 운영 Supabase에 먼저 적용해야 한다. Codex는 migration 파일만 만들었으며 운영 DB에는 적용하지 않았다.
- 새 환경변수와 외부 패키지는 없다.
- 운영 반영에는 앱 재배포가 필요하다.

### 남은 위험
- 월간 집계는 현재 반 소속 기준이다. 반 이동 당시 반을 기준으로 보려면 `attendance_records.class_id` snapshot과 기존 데이터 backfill 정책을 별도 설계해야 한다.
- 현재 공개 anon DB와 localStorage 선생님 로그인 구조의 권한 한계는 유지된다. 실제 반별 접근 제한은 Supabase Auth/RLS 전환이 필요하다.
