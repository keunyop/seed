...
- 아이 상세 정보와 보호자 연락처, 선택한 사진 Data URL도 Supabase 정규화 테이블에 저장합니다. 아이·선생님 사진은 `사진 찍기`와 `앨범에서 선택`을 분리해 제공하고, 160KB를 넘는 휴대폰 사진은 브라우저에서 자동 축소한 뒤 저장합니다.
- 생년월일을 모르는 아이는 생일 필드를 비워 등록할 수 있으며, 월간 생일자 통계에서는 제외됩니다.
- 통계는 최근 8주 출석자와 현재 연도 월별 큐티 완료자·생일자를 막대그래프로 보여주며, 막대별 아바타/목록 상세와 모든 선생님 메모 통합 목록을 제공합니다.
