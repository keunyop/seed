...
export function StatCard({ icon: Icon, label, value, helper, onClick }: StatCardProps) {
  const content = (
    <>
  ...
  if (onClick) {
    return (
      <button
        className={cn(
          "rounded-[12px] border-2 border-cloud-gray p-4 text-left transition hover:border-duo-green focus-visible:border-sky-blue",
        )}
        onClick={onClick}
        type="button"
      >
